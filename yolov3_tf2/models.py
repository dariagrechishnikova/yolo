...
flags.DEFINE_integer('yolo_max_boxes', 780,
                     'maximum number of boxes per image')
flags.DEFINE_float('yolo_iou_threshold', 0.5, 'iou threshold')
flags.DEFINE_float('yolo_score_threshold', 0.5, 'score threshold')

yolo_anchors = np.array([(9, 11), (11, 19), (16, 33), (18, 16), (26, 63),
...
def yolo_boxes(pred, anchors, classes):
    #postprocess prediction for loss(pred_box) and for nms and final output
    # pred: (batch_size, grid, grid, anchors, (x, y, w, h, obj, ...classes))
    grid_size = tf.shape(pred)[1:3]
    #split volume by chunks using last dim.Ex: bbox_xy shape [batch_size, grid, grid, anchors, 2]
    box_xy, box_wh, objectness, class_probs = tf.split(
        pred, (2, 2, 1, classes), axis=-1)

    #bring values to range [0,1]
    ...
    box_wh = tf.exp(box_wh) * anchors #net predicts w and h as some transformation of anchor boxes(offsets)
